...
func ParseSqlFile(filename string) (*Schema, error) {
	...
	for scanner.Scan() {
		...
		if strings.Contains(line, "CREATE TABLE") {
			...
			if slices.Contains(config.C.DatabaseConfig.IgnoreTables, tableName) {
				continue
			}
			if mustFilter {
				if !slices.Contains(checkColumnStr, tableName) {
					continue
				}
			}
		...
		if line[0] == '`' {
			...
			if slices.Contains(config.C.DatabaseConfig.IgnoreColumns, fieldName) {
				continue
			}
			fieldComment := ""
