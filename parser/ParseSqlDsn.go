...
func ParseSqlDsn(dsn string) (*Schema, error) {
	...
	for rows.Next() {
		...
		if slices.Contains(config.C.DatabaseConfig.IgnoreTables, table) {
			continue
		}
		if mustFilter {
			if !slices.Contains(checkColumnStr, table) {
				continue
			}
		}
	...
	for _, column := range columns {
		if slices.Contains(config.C.DatabaseConfig.IgnoreColumns, column.ColumnName) {
			continue
		}
