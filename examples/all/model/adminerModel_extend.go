...
type adminer_model interface {
	...
	FindByResume(ctx context.Context, resume sql.NullString) (*Adminer, error)
	...
	FindsByResumes(ctx context.Context, resumes []sql.NullString) ([]*Adminer, error)
	...
	FindsByResume(ctx context.Context, resume sql.NullString) ([]*Adminer, error)
...
func (m *defaultAdminerModel) FindList(ctx context.Context, pageSize, page int64, keyword string, adminer *Adminer) (resp []*Adminer, total int64, err error) {
	...
	if adminer != nil {
		...
		if adminer.Resume.String != "" {
			sq = sq.Where("`resume` = ?", adminer.Resume)
		}
		if adminer.Status >= 0 {
		...
		if adminer.LastLogin.After(time.Unix(0, 0)) {
...
func (m *defaultAdminerModel) FindByResume(ctx context.Context, resume sql.NullString) (*Adminer, error) {
	var resp Adminer
	query := fmt.Sprintf("select %s from %s where `resume` = ? and `is_deleted` = '0' limit 1", adminerRows, m.table)
	err := m.conn.QueryRowCtx(ctx, &resp, query, resume)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
...
func (m *defaultAdminerModel) FindsByEmails(ctx context.Context, emails []string) ([]*Adminer, error) {
	...
	agr := strings.Join(emails, ",")
	query := fmt.Sprintf("select %s from %s where `email` in (?) and `is_deleted` = '0' ", adminerRows, m.table)
	err := m.conn.QueryRowsCtx(ctx, &resp, query, agr)
	return resp, err
}

func (m *defaultAdminerModel) FindsByResumes(ctx context.Context, resumes []sql.NullString) ([]*Adminer, error) {
	var resp = make([]*Adminer, 0)
	if len(resumes) == 0 {
		return resp, nil
	}
	...
	agr := strings.Join(strs, ",")
	query := fmt.Sprintf("select %s from %s where `resume` in (?) and `is_deleted` = '0' ", adminerRows, m.table)
	err := m.conn.QueryRowsCtx(ctx, &resp, query, agr)
	return resp, err
}
...
func (m *defaultAdminerModel) FindsByResume(ctx context.Context, resume sql.NullString) ([]*Adminer, error) {
	var resp = make([]*Adminer, 0)
	query := fmt.Sprintf("select %s from %s where `resume` = ? and `is_deleted` = '0' ", adminerRows, m.table)
	err := m.conn.QueryRowsCtx(ctx, &resp, query, resume)
	return resp, err
}
